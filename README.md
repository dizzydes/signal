...
| `apps/patient-web` | Express | "Sigup" typo button, `/api/status` honouring `FAILURE_RATE`, `/heartbeat`, optional `MEMORY_LEAK` |
...
2. Create four services pointing at this repo, each with **Root Directory** set to its package folder and **Config File Path** left as default (`railway.json` at the root directory):
...
4. Set sealed variables on services that need them (see `.env.example`):
   ...
   - `patient-web`: `FAILURE_RATE` (starts at 0), optional `MEMORY_LEAK=0`
...
- **Break the build** — commits to main removing `chalk` from `apps/patient-web/package.json`. Railway build fails. Poller detects the failed deployment. Healer restores the dep.
...
Resets patient sources to the `demo-base` tag, clears signals older than N minutes, and restores `FAILURE_RATE=0` on Railway. See `scripts/reset.ts`.
